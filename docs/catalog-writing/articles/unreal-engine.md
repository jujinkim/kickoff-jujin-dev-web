...
**Question:** How do Actors, components and Blueprint events define a collectible?

**Include:** Explain Actors in a level and a reusable Blueprint class with component composition.

**Exclude:** Rendering feature comparisons, royalties and supported-platform promises.

**Prevent confusion:** Blueprint graphs describe behavior; they are not merely decorative flowcharts.

**Terms (EN / KO / JA):** Unreal Engine / Unreal Engine / Unreal Engine; Actor / 액터 / アクター. Also use the [group terms](../groups/game-engines.md) and [shared glossary](../glossary.md). These are editorial translations, not claims of standardized terminology.

## Six-section article outline

### 1. Definition

Open by answering the reader question. Establish this boundary: Explain Actors in a level and a reusable Blueprint class with component composition. Explain the distinguishing mechanism: Actors hold components; Blueprint graphs can define gameplay [unreal] [blueprints]. Immediately separate the nearby concept: Blueprint graphs describe behavior; they are not merely decorative flowcharts.
...
Input: level with player and collectible Actor. Initial: score 0. Action: overlap event invokes authored score increment and removes the Actor. Result: score 1.

### 3. Advantages and limitations

Explain the conditional benefit: The example exposes event-to-score behavior as an inspectable graph. Then show its boundary: Growing graphs and cross-object references still require clear ownership. Use the unsuitable case below as a counterexample; do not turn editorial advice into an empirical performance or usability result.
...
**Suitable:** A team using visual gameplay scripting and reusable Actor classes.

**Unsuitable:** Assuming visual scripting removes the need to design state and failure behavior.

**Combine:** Combine Blueprint interaction with separately owned score logic; C++ extension is optional scope context only.
...
| Field          | Planned point                                                                                                  |
| -------------- | -------------------------------------------------------------------------------------------------------------- |
| `features`     | Actors hold components; Blueprint graphs can define gameplay [unreal] [blueprints].                            |
| `advantages`   | The example exposes event-to-score behavior as an inspectable graph.                                           |
| `limitations`  | Growing graphs and cross-object references still require clear ownership.                                      |
| `suitable`     | A team using visual gameplay scripting and reusable Actor classes.                                             |
| `combinations` | Combine Blueprint interaction with separately owned score logic; C++ extension is optional scope context only. |
...
**Purpose:** Answer “How do Actors, components and Blueprint events define a collectible?” through the fixture rather than a decorative illustration.
...
**Interaction/state script:** Input: level with player and collectible Actor. Initial: score 0. Action: overlap event invokes authored score increment and removes the Actor. Result: score 1.

**Annotations, caption and topic acceptance:** Show level enclosure, Actor components and event→increment→remove graph. Caption: Component composition and event logic work together. Accept if the graph has one guarded collection path, preventing duplicate scoring.
