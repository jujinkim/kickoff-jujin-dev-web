# Priority queue / 우선순위 큐 / 優先度付きキュー

Writing design only. ID: `priority-queue` · Group: [processing-order](../groups/processing-order.md) · Queue order: 602 · Checked: 2026-09-21.

## Reader question and scope

**Question:** How does an explicit priority change the next task?

**Include:** Explain priority-based removal with a chosen comparator and tie rule.

**Exclude:** Heap implementation tutorials and universal stable-tie guarantees.

**Prevent confusion:** Priority queues need not iterate in sorted order or preserve equal-priority arrival order automatically.

**Terms (EN / KO / JA):** Priority queue / 우선순위 큐 / 優先度付きキュー; Comparator / 비교자 / コンパレーター. Also use the [group terms](../groups/processing-order.md) and [shared glossary](../glossary.md). These are editorial translations, not claims of standardized terminology.

## Six-section article outline

### 1. Definition

Open by answering the reader question. Establish this boundary: Explain priority-based removal with a chosen comparator and tie rule. Explain the distinguishing mechanism: Removal follows priority rather than arrival [orders]. Immediately separate the nearby concept: Priority queues need not iterate in sorted order or preserve equal-priority arrival order automatically.
...
Input: A(2),B(1),C(2). Initial: empty. Action: add all; remove minimum by (priority,arrival sequence). Result: B,A,C, then Empty.

### 3. Advantages and limitations

Explain the conditional benefit: The authored urgent task B can run before A. Then show its boundary: Lower-priority work can wait indefinitely unless scheduling policy addresses it. Use the unsuitable case below as a counterexample; do not turn editorial advice into an empirical performance or usability result.

### 4. Comparison within the category

Compare every sibling: [stack](stack.md), [queue](queue.md).

Use the shared axes, in this order: **next-removal rule; tie policy; empty behavior; starvation risk; possible backing structure**. Populate the five comparison fields below from this same argument. Read the [group matrix](../groups/processing-order.md) before drafting peer comparisons; compare the same dimensions and assumptions while each published article uses its own familiar situation.

### 5. Selection and combination conditions

**Suitable:** Work where priority has a clear domain meaning.

**Unsuitable:** Using arbitrary priority values with no tie or starvation policy.

**Combine:** Implement with a heap; add an arrival sequence to the comparator when stable ties are required.
...
| Field          | Planned point                                                                                   |
| -------------- | ----------------------------------------------------------------------------------------------- |
| `features`     | Removal follows priority rather than arrival [orders].                                          |
| `advantages`   | The authored urgent task B can run before A.                                                    |
| `limitations`  | Lower-priority work can wait indefinitely unless scheduling policy addresses it.                |
| `suitable`     | Work where priority has a clear domain meaning.                                                 |
| `combinations` | Implement with a heap; add an arrival sequence to the comparator when stable ties are required. |
...
**Purpose:** Answer “How does an explicit priority change the next task?” through the fixture rather than a decorative illustration.
...
**Interaction/state script:** Input: A(2),B(1),C(2). Initial: empty. Action: add all; remove minimum by (priority,arrival sequence). Result: B,A,C, then Empty.

**Annotations, caption and topic acceptance:** Show numeric priority and sequence, not just colors. Caption: Priority selects first; this example breaks ties by arrival. Accept if the tie policy is stated rather than attributed to every implementation.
