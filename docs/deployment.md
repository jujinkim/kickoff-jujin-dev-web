...
## Repository configuration and historical setup
...
## Historical public origin — 2026-09-21
...
1. Run checks sequentially: content/type/format → build → thumbnails → rebuild → artifact tests → browser tests against `dist`. Instruction-only changes can skip thumbnail capture and the extra build.
