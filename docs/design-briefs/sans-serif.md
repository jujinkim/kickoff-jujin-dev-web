# Sans serif visualization brief

Based on [the planning template](../templates/design-demo-brief.md). Current article and visual reviewed 2026-09-23.

- Stable ID, leaf category, English/Korean/Japanese titles: `sans-serif`; `type-shapes`; Sans serif / 산세리프 / サンセリフ.
- Definition (one sentence): Sans-serif letters omit serifs, but their terminals and proportions still vary.
- Closest concept and concrete difference: Serif adds terminal strokes; this specimen uses plain terminals at quick-scan label size.
- Distinguishing visual features (structure, material, typography): A specimen shows plain terminal strokes at the size used for quick labels.
- Comparison category: `type-shapes`; comparison dimensions are shared, but this article uses its own situation.
- Distinct familiar situation, dataset, labels, actions, and initial state for this article: train information board; Localized specimen, 48px size, guides hidden and number alignment at its default.
- Visual decisions and controls that demonstrate this concept: A specimen shows plain terminal strokes at the size used for quick labels. Specimen input, size, measured width guides, number alignment and Reset.
- Fictional scenario and why it demonstrates the definition: For a train information board, an interface needs a restrained letter style with fewer terminal ornaments.
- Representative action and observable result: Change weight and size: compare the same label while preserving readable fallback.
...
- Capture selector and initial content: `[data-demo="sans-serif"]`; three localized PNGs at `public/thumbnails/sans-serif-{en,ko,ja}.png`.
