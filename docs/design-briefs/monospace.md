# Monospace visualization brief

Based on [the planning template](../templates/design-demo-brief.md). Current article and visual reviewed 2026-09-23.

- Stable ID, leaf category, English/Korean/Japanese titles: `monospace`; `character-width`; Monospace / 고정폭 / 等幅.
- Definition (one sentence): Monospaced Latin glyphs share an advance width even when their ink shapes differ.
- Closest concept and concrete difference: Proportional text gives supported letters varied advances; monospace makes their advances equal.
- Distinguishing visual features (structure, material, typography): Supported Latin letters occupy equal advances for aligned records; fallback may differ.
- Comparison category: `character-width`; comparison dimensions are shared, but this article uses its own situation.
- Distinct familiar situation, dataset, labels, actions, and initial state for this article: weather log; Localized specimen, 48px size, guides hidden and number alignment at its default.
- Visual decisions and controls that demonstrate this concept: Supported Latin letters occupy equal advances for aligned records; fallback may differ. Specimen input, size, measured width guides, number alignment and Reset.
- Fictional scenario and why it demonstrates the definition: For a weather log, code and text tables rely on aligned positions.
- Representative action and observable result: Measure i and W and align numbers: supported Latin columns stay even.
...
- Capture selector and initial content: `[data-demo="monospace"]`; three localized PNGs at `public/thumbnails/monospace-{en,ko,ja}.png`.
...
- Comparison summaries: features `Monospaced Latin glyphs share an advance width even when their ink shapes differ.`; advantages `Equal Latin advances preserve code and data column alignment.`; limitations `CJK, emoji, combining marks, and fallback faces can break that model; inspect the supported character set.`; suitable `Choose it for code or aligned Latin data.`; combinations `Use fixed-width code beside proportional explanatory text.`.
