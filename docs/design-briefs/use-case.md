...
- Article-specific scenario, dataset, labels, actions, and initial state: home cook, Weekend Recipes unsaved, link needed for later reading; storage initially available when evaluating saving. Save is a discussed action, not a connected feature. Saving a link does not promise offline reading.
...
- Fictional scenario and why it demonstrates the definition: A home cook saves Weekend Recipes, but a failed write could leave the screen saying Saved with no record to return to. Weekend Recipes is unsaved; storage is available.
- Representative action and observable result: Weekend Recipes is unsaved; storage is available.
- Initial state: the full labeled diagram is rendered on the server; the fictional starting state is Weekend Recipes unsaved.
...
- Localized visible strings, input constraints, original/translation review: English group reviewed before Korean/Japanese translation on 2026-09-22. Weekend Recipes, roles, branches and offline boundary match. No inputs or native-speaker certification.
