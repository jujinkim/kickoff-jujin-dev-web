# Tactile collage / 촉각적 콜라주 / 触感的コラージュ

Based on [the planning template](../templates/design-demo-brief.md). Implementation brief, reviewed 2026-09-22.

- Stable ID, leaf category, English/Korean/Japanese titles: `tactile-collage`; `styles`; Tactile collage / 촉각적 콜라주 / 触感的コラージュ.
- Definition (one sentence): Tactile collage layers paper, tape, and annotations. This editorial category groups handmade cues discussed in Canva’s 2026 trends; it does not date their invention.
- Closest concept and concrete difference: Skeuomorphism models one physical notebook; this study layers fragments from different materials.
- Distinguishing visual features (structure, material, typography): Layered paper fragments, tape, irregular edges, and annotations. A board of layered papers, tape and annotations. Paper offsets stay inside the board; reading order remains overview, tasks, progress, resources.
- Shared comparison category (styles / layout / typography): styles.
- Distinct familiar situation, dataset, labels, actions, and initial state for this article: For the memory board, track three tasks and two notes.
- Visual variable changed; concept-specific extra controls and their justification: Layered paper, tape, and a completion mark turn the workspace into a board. No extra task actions.
- Fictional scenario and why it demonstrates the definition: A memory board should preserve the feel of assembled photos and tickets. For the memory board, track three tasks and two notes.
- Representative action and observable result: For the memory board, track three tasks and two notes. Mark one task: progress becomes 1 / 3 and 33%.
...
- Capture selector and initial content: `[data-demo="tactile-collage"]`; full root in all languages, light theme, reset state, cleared live message. PNG remains complete; catalog card uses top-aligned cover with 360px maximum.
...
- Comparison summaries: features: Layered paper fragments, tape, irregular edges, and annotations. Advantages: Material cues give an informal board a personal character. Limitations: Overlap and texture must not obscure controls or text. Suitable: Choose it for a small creative project board. Combinations: Keep semantic order and quiet text surfaces beneath the collage.
