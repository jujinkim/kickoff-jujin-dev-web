# Multiple columns visualization brief

Based on [the planning template](../templates/design-demo-brief.md). Current article and visual reviewed 2026-09-23.

- Stable ID, leaf category, English/Korean/Japanese titles: `multiple-columns`; `columns`; Multiple columns / 다열 / マルチカラム.
- Definition (one sentence): Three or more page regions keep navigation, active work, and context nearby.
- Closest concept and concrete difference: Two columns gives one supporting side region; this example gives filters, exhibits and notes three peer regions.
- Distinguishing visual features (structure, material, typography): Filters, exhibit references and study notes occupy three peer regions when space permits.
- Comparison category: `columns`; comparison dimensions are shared, but this article uses its own situation.
- Distinct familiar situation, dataset, labels, actions, and initial state for this article: museum guide; Six labeled visual references, collection order, all topics and notes closed.
- Visual decisions and controls that demonstrate this concept: Filters, exhibit references and study notes occupy three peer regions when space permits. Topic filter, collection order, study-note disclosure, preview width and Reset.
- Fictional scenario and why it demonstrates the definition: For a museum guide, museum visitors need the route, exhibit list, and notes together.
- Representative action and observable result: Narrow the preview: all three regions stack in source order.
...
- Capture selector and initial content: `[data-demo="multiple-columns"]`; three localized PNGs at `public/thumbnails/multiple-columns-{en,ko,ja}.png`.
