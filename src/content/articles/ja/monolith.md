...
近所の人が料理本を探して保存し、追加機能を購入するレシピアプリを作るとします。少人数の一チームが保存した本に家族タグを加えたいものの、別々のリリース調整は負担です。機能別の日程より一括更新を優先します。

## どう解決するのか

1. 一チームがカタログ・ライブラリ・課金を一括更新します。保存した地域の料理本にはタグがありません。
2. ライブラリにタグ機能を加え、アプリv2をデプロイ。課金の動作は同じでも同じ成果物に含まれる。共有データベースはアプリが所有。
3. ライブラリがプロセス内でカタログを参照し、`家族`を記録。書き込み前の参照失敗ならタグなしのまま。復旧後に再試行。
