---
kind: concept
articleId: masonry
lang: ja
title: メイソンリー
summary: 旅のアルバムの視覚資料を配置します。
category: content-arrangement
aliases:
  - メイソンリー
related:
  - layout
  - list-layout
  - uniform-grid
status: published
revision: 4
sourceRevision: 4
updated: "2026-09-23"
comparison:
  features: 高さの違うカードを、その時点で最も短い列に置きます。
  advantages: 短い列がカードを受け入れ、共通行の隙間を減らします。
  limitations: 展開後のキーボード順と重なりを確認し、狭い画面では1列にします。
  suitable: 視覚資料の収集に向きます。
  combinations: 最小限のカード装飾で多様な画像を引き立てます。
---

## なぜ必要なのか

旅のアルバムには高さの違う縦横の写真があります。行の高さをそろえると、余白が増えたり必要な情報を切り取ったりします。

## どう解決するのか

旅のアルバムを設計しながら、視覚資料を6件集めます。テーマの絞り込み、収集順の反転、研究メモ、表示幅を操作します。資料を探す流れが明確か確かめます。リセット・再読み込みで全件と収集順に戻ります。 高さの異なるカードが短い列を埋め、メモを開くとDOM順を保って再配置します。

## どんな考え方なのか

**メイソンリー** — 高さの違うカードを、その時点で最も短い列に置きます。

展開後のキーボード順と重なりを確認し、狭い画面では1列にします。

[W3C](https://www.w3.org/WAI/WCAG22/Understanding/reflow.html)
