...
記事を読む操作はファイルの経路を通ります。地域の便りの保存は別のAPIから外部ストアへ進みます。再保存しても例の読者・記事キーにより記録は1件です。次の保存を失敗させるとストアを変えず失敗を返します。処理器を再起動しても外部の記録は残ります。リセット・再読み込みは仮想ストアを含むページメモリ全体を消します。
