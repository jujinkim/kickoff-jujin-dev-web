---
kind: concept
articleId: brutalism
lang: ja
title: ブルータリズム
summary: 同じプロジェクト作業室でスタイルを比較.
category: styles
aliases:
  - ブルータリズム
related:
  - theme
  - neobrutalism
  - glassmorphism
  - neumorphism
  - skeuomorphism
  - flat-design
  - minimalism
status: published
revision: 9
sourceRevision: 9
updated: "2026-09-23"
comparison:
  features: 露出した罫線と無骨な文字が構造を示します。 太い区切りと下線付きメニューで構造を示します。
  advantages: むき出しの区切り線でタスクの境界を示します。
  limitations: 粗い印象が邪魔にならないよう、リンクの下線と明確な操作を保ちます。
  suitable: 実用的なタスク一覧に適しています。
  combinations: リストや単一列でタスクを順に確認できます。
checked: "2026-09-21"
---

## なぜ必要なのか

来場者が修理場所とボランティア作業を探す案内を作るとします。装飾が急ぎの場所一覧を隠します。運営側は静かに要素を減らす画面より、むき出しの区切り線と下線リンクを選びます。

## どう解決するのか

修理会の作業三件とメモ二件で一件を完了すると、進捗は1 / 3、33%になります。絞り込みやメモ展開を試し、リセットで初期状態と比べます。太い区切りと下線付きメニューで構造を示します。

## どんな考え方なのか

**ブルータリズム** — 露出した罫線と無骨な文字が構造を示します。

粗い印象が邪魔にならないよう、リンクの下線と明確な操作を保ちます。

[NN/g](https://www.nngroup.com/articles/brutalism-antidesign/)
