---
kind: concept
articleId: neobrutalism
lang: ja
title: ネオブルータリズム
summary: 同じプロジェクト作業室でスタイルを比較.
category: styles
aliases:
  - ネオブルータリズム
related:
  - theme
  - brutalism
  - glassmorphism
  - neumorphism
  - skeuomorphism
  - flat-design
  - minimalism
status: published
revision: 6
sourceRevision: 6
updated: "2026-09-23"
comparison:
  features: 鮮やかな色面、太い枠、硬いオフセット影で強調します。 太い枠と硬い影でパネルを強調します。
  advantages: 強い色面でタスクと操作を強調します。
  limitations: 装飾が詳細と競合するため、強調は重要な情報に絞ります。
  suitable: 大胆な印象の小さな画面に適しています。
  combinations: 繰り返すタスク行を簡潔に保ちます。
checked: "2026-09-21"
---

## なぜ必要なのか

小さな画面に強い個性と明確な区切りを付けたい。わずかな面の違いだけでは強調が足りない場合があります。

## どう解決するのか

作業三件とメモ二件の画面で一件を完了すると、進捗は1 / 3、33%になります。絞り込みやメモ展開を試し、リセットで初期状態と比べます。太い枠と硬い影でパネルを強調します。

## どんな考え方なのか

**ネオブルータリズム** — 鮮やかな色面、太い枠、硬いオフセット影で強調します。

装飾が詳細と競合するため、強調は重要な情報に絞ります。

[NN/g](https://www.nngroup.com/articles/neobrutalism/)
