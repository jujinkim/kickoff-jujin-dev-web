...
購入した機能は繰り返し使っても残るべきです。利用ごとに数量を減らす仕組みでは、その約束に合いません。

## どう解決するのか

架空のパズルアプリは夜のテーマがロックされた状態で始まります。一度買って繰り返し適用しても権利は残り、購入回数は1です。この例の再購入操作では再請求しません。実際の購入復元は実装範囲外です。価格・税金・手数料・返金は省略し、リセットは例だけを消去します。
