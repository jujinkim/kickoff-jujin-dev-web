...
등산객이 안내 글을 저장하고 부가 기능을 구매하는 앱을 만든다고 가정해 봅시다. 주말 태그를 추가하다 관련 없는 결제 코드까지 바꿀 위험이 있습니다. 작은 팀은 하나의 배포 안에서 소유권을 명확히 하고 싶으며 별도 서비스 운영 부담은 원하지 않습니다.

## 어떻게 해결하는가

1. 한 팀이 카탈로그·보관함·결제를 함께 배포합니다. 저장한 등산 안내에는 태그가 없습니다.
2. 보관함 소유 테이블에 태그 저장을 추가하고 앱 v2를 배포합니다. 결제 동작은 그대로입니다. 데이터베이스는 하나지만 테이블은 모듈별 소유, 다른 모듈의 직접 접근은 금지합니다.
3. 보관함이 카탈로그 API를 프로세스 내부에서 호출한 뒤 `주말`을 기록합니다. 조회 실패 시 태그 없음 유지, 복구 후 재시도.
