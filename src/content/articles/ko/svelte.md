...
여행자가 버스와 등산 카드를 저장하는 계획 앱을 만든다고 가정해 봅시다. 문구와 합계가 함께 갱신돼야 합니다. 제작팀은 갱신 코드를 많이 쓰지 않고 선언형 컴포넌트를 미리 컴파일하려 합니다.

## 어떻게 해결하는가

여행 계획 카드 2개가 미저장 상태로 시작합니다. 버스 시간표 저장을 누르면 해당 카드만 저장됨으로 바뀌고 등산 경로는 그대로입니다. 등산 경로도 저장하면 총수가 1에서 2가 됩니다. 같은 카드를 다시 저장해도 총수는 2입니다. 도식은 컴파일과 실행 중 갱신을 구분합니다. 초기화·새로고침은 두 카드를 지웁니다. 영속 저장은 제공하지 않습니다.
